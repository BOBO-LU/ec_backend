        ...
        for (let key in deleteList) {
            ...
            if (hasData === false) {
                ...
                db.query(`
                        DELETE 
                        FROM order_list 
                        WHERE order_id = ? and member_id = ? and product_id = ?`,
                        [deleteList[key].orderID, deleteList[key].memberID, deleteList[key].productID],
...
const checkOrderComplete = function (orderID, memberID, productID) {
    return new Promise((resolve, reject) => {
        db.query(`
                SELECT * 
                FROM order_list 
                WHERE order_id = ? AND member_id = ? AND product_id = ? AND is_complete = 0`,
                [orderID, memberID, productID], function (err, rows) {
            if (rows[0] === undefined) {
                resolve(false);
            } else {
                resolve(true);
